                    ...
                    return c.cb_comment_is_deleted === true ? <div id='delete_comment_box' key={i}>* 삭제된 댓글입니다 * <br /></div> :
                        <div id='comment_box' key={i}>
                            <div id='comment_writer_box1'>
                                ...
                                {c.company_id === loginId && adAc !== "adminAccess" && <button id='delete_comment_button' onClick={() => axiosDeleteComment(c.comment_no)}>삭제</button>} <br />
                            </div>
                            <div id='comment_text_box'>{c.cb_comment_text}</div> <div id='comment_date_box'>{c.formattedDate}</div>
                        </div>
                }
                )}
                {/* ↓ 이렇게도 쓸 수 있음
