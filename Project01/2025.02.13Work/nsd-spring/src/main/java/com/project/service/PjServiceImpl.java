		...
		for (LoginDto l : lo) {
			for (ProjectManagementDto p : pl) {
				for (ProjectMemberDto pm : pro) {
					...
					if (p.getProject_no() == pm.getProject_no() && l.getCompany_id().equals(pm.getCompany_id())
							&& !endedPjAl.contains(pm.getProject_no()) && !p.getEmployee().contains(l)) {
						// 이미 추가된 직원인지 확인
						p.getEmployee().add(l);
					}
				}
...
//		}
	...
	public void changePosition(LoginDto l) {
		...
		LoginDto cp = new LoginDto(l.getCompany_id(), position, l.getPosition_rank());
	...
	public ArrayList<ProjectManagementDto> getEndProjectList() {
		ArrayList<ProjectManagementDto> pl = mapper.getEndProjectList();
		ArrayList<LoginDto> lo = mapper.getEmployeeList();
		ArrayList<ProjectMemberDto> pro = mapper.getProjectMemberlist();
		...
		for (LoginDto l : lo) {
			for (ProjectManagementDto p : pl) {
				for (ProjectMemberDto pm : pro) {
					if (p.getProject_no() == pm.getProject_no() && 
						l.getCompany_id().equals(pm.getCompany_id()) && 
						!p.getEmployee().contains(l)) {
						p.getEmployee().add(l);
					}
				}
			}
