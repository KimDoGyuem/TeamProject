...
function IssueRead() {
    ...
    function axiosIssueClose(e) {
        ...
        if(window.confirm(`"${issue.issue_no}"번 이슈를 닫으시겠습니까?`)){
            axios.patch(`${LocalUrl()}/company/issueClose`, body)
                .then(()=>{
                    alert('!!issue Closed!!')
                    axiosGetIssueByIssueNo();
                })
    ...
    function axiosIssueReOpen(e) {
        ...
        if(window.confirm(`"${issue.issue_no}"번 이슈를 재오픈하시겠습니까?`)){
            axios.patch(`${LocalUrl()}/company/issueReOpen?issueNo=${issue_no}`)
                .then(()=>{
    ...
    function handleSubmitEdit(e) {
        ...
        axios.patch(`${LocalUrl()}/company/issueCommentModify`, editComment)
            .then(() => {
                setEditComment({ comment_no: 0, comment_text: '' });
                axiosGetIssueByIssueNo();
            })
    ...
    const handleCommentSubmit = (e) => {
        ...
        axios.post(`${LocalUrl()}/company/issueCommentWrite`, newComment)
            .then(() => {
                setComment('');
                axiosGetIssueByIssueNo();
            })
            ...
            <div className="issue-detail-container">
                <div className="issue-header">
                    <div className="issue-title-section">
                        <h1>
                            {issue.issue_title}
                            <span className="issue-number">#{issue.issue_no}</span>
                            {issue.issue_label && (
                                <span className={`issue-label label-${LABEL_MAP[issue.issue_label]?.className || issue.issue_label.toLowerCase()}`}>
                                    {LABEL_MAP[issue.issue_label]?.display || issue.issue_label}
                                </span>
                            )}
                        </h1>
