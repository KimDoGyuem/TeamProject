...
function EmployeeManagement() {
    ...
    function axiosGetDepartmentList() {
        axios.get(`${LocalUrl()}/company/getDepartmentList`)
        .then(response => {
    ...
    function axiosAddDepartment(e) {
        e.preventDefault();
        axios.post(`${LocalUrl()}/company/addDepartment?name=${departmentName}`)
        .then(() => {
    ...
    function deleteEmployee(id) {
        axios.delete(`${LocalUrl()}/company/deleteEmployee?id=${id}`)
        .then(() => {
    ...
    function changePosition(b1) {
        axios.patch(`${LocalUrl()}/company/changePosition`, b1)
        .then(() => {
            axiosGetDepartmentList();
            axiosGetEmployeeList();
        })
    ...
    function departmentMemberAdd(b2) {
        axios.patch(`${LocalUrl()}/company/departmentMemberAdd`, b2)
            .then(() => {
                axiosGetDepartmentList();
                axiosGetEmployeeList();
            })
        ...
        function emExclude(id) {
            axios.patch(`${LocalUrl()}/company/emExclude?id=${id}`)
            .then(() => {
                axiosGetDepartmentList();
            })
            .catch(error => {
        ...
        function deleteDepartment(departmentName) {
            axios.delete(`${LocalUrl()}/company/deleteDepartment?department=${departmentName}`)
            .then(() => {
                axiosGetDepartmentList();
                axiosGetEmployeeList();
            })
        ...
        axios.patch(`${LocalUrl()}/company/departmentLeader`, b)
        .then(() => {
            axiosGetDepartmentList();
        })
        .catch(error => {
    ...
    function addEmployee(e){
        e.preventDefault();
        // 직급에 따른 랭크 정하기
        const p = {'사원':7, '주임':6, '대리':5, '과장':4, '차장':3, '부장':2, '사장':1};
        let rank = p[position] ?? 7; // 객체의 프로퍼티 접근 방법 p['key'] 는 해당 key의 value 값이 나옴 / null, undefined 가 나올 경우 오른쪽 값(2)을 반환
        
        let body = {company_name:addName, company_id:addId, position:position, position_rank:rank};
        axios.post(`${LocalUrl()}/company/addEmployee`, body)
            .then(() => {
                setaddName('');
                setaddId('');
                setPosition('사원');
