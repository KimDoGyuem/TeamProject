...
function BoardManagement({ readPage }) {
    ...
    function axiosGetAllPostList() {
        axios.get(`${LocalUrl()}/companyBoard/getAllPostList?searchTag=${searchTag}&searchWord=${searchWord}&del=${del}&searchCategory=${searchCategory}`)
            .then((response) => {
    ...
    function recoverPost() {
        ...
        axios.patch(`${LocalUrl()}/companyBoard/recoverPost`, { postNos: selectPosts }) //  키(key)는 'postNOs'이고 값(value)은 selectPosts배열을 담은 객체형태로 보냄
            .then(() => {
    ...
    function dataBaseDelPost() {    // 글 삭제시 해당 글에 달린 댓글 까지 전부 DB에서 삭제처리
        ...
        axios.delete(`${LocalUrl()}/companyBoard/dataBaseDelPost`, { data: { postNos: selectPosts } })
            .then(() => {
                setSelectPosts([]);
                axiosGetAllPostList();
            })
    ...
    function addBoardCategory(e) {
        ...
        if (isDuplicateCategory) {
            ...
            axios.post(`${LocalUrl()}/companyBoard/addCategory?name=${addCategory}`)
                .then(() => {
                    setAddCategory('');
                    axiosGetCategoryList();
                })
    ...
    function deleteCategory(name) {
        ...
        axios.delete(`${LocalUrl()}/companyBoard/deleteCategory?name=${name}`)
            .then(() => {
                axiosGetAllPostList();
                axiosGetCategoryList();
            })
    ...
    function onTtrendingPeriod(tp) {
        axios.patch(`${LocalUrl()}/companyBoard/onTtrendingPeriod?tp=${tp}`)
            .then(() => {
                currentTrendingPeriod();
            })
            .catch(error => {
