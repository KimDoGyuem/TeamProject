...
function BoardManagement({ readPage }) {
    ...
    function checkBoxSelect(post) {
        ...
        setSelectPosts((prevSelected) => {  // 화살표 함수 사용
            if (prevSelected.includes(cb_no)) {  // includes는 배열에 no가 존재하는지 확인하는 함수(존재 할 경우 true)
                return prevSelected.filter((i) => i !== cb_no);    // 배열에서 no를 지운 새로운 배열 리턴
            } else {
                return [...prevSelected, cb_no];   // 배열에 no가 없을 경우 기존배열을 복사하여 no를 추가
            }
        });
        //선택 객체 정보 배열 업데이트
    ...
    function recoverPost() {
        const is = selectPostInfos.every((p) => p.cb_is_deleted);
            ...
            <div id='management_box'>
                ...
                <div id='button_box'>
                    ...
                    <div id='util_button_box'>
                        <button className={selButton ? 'select_button' : ''} onClick={allView}>전체글 보기</button>
                        <button className={selButton ? '' : 'select_button'} onClick={delView}>삭제글 보기</button>
                        <button id='delete_button' onClick={dataBaseDelPost}>선택글 삭제</button>
                        {/* {del === true && <button id='recover_button' onClick={recoverPost}>선택글 복구</button>} */}
