...
function EmployeeManagement() {
    ...
    function closePopup(employeeId, positionRank, departmentName) {
        if (employeeId !== 'x' && positionRank > 0 && departmentName === '') {   //직급 변경 
            let b1 = { company_id: employeeId, poistion_rank: positionRank };
            changePosition(b1);
        }
        if (employeeId !== 'x' && positionRank < 0 && departmentName !== '') {   //부서 변경
            let b2 = { company_id: employeeId, department_name: departmentName };
    ...
    function addEmployee(e){
        ...
        let body = {company_name:addName, company_id:addId, position:position, p_rank:rank};
