...
function ProjectList({pjData, projectDelete, loginRank, onExclude, loginId, onDrop}){    // 프로젝트 리스트와 참여 인원 리스트 컴포넌트
    ...
    const handleDrop = (e) => {
        ...
        onDrop({ company_id: employee.company_id, project_number: pjData.no });
    };
        ...
        <div>
            ...
            >
                <legend>
                    ...
                    {(loginRank === 0 || pjData.employee.some(emp => emp.company_id === loginId)) && 
                        <Link to={`/issueWrite/${pjData.project_no}`}><button>이슈 등록</button></Link>
                    }
                    {isIssue && IssueListButton(pjData.project_no)}
                    {loginRank<=0 && <button onClick={()=>projectDelete(pjData.project_no, pjData.project_name)}>프로젝트 종료</button>}
                </legend>
