        ...
        <div id='board_modify_page'>
            <div id='modify_box'>
                <form onSubmit={axiosModifyPost}>
                    <input style={{ width: '1000px', height: '50px' }} name='cb_title' value={post.cb_title} onChange={handleChange}></input> <br /><br />
                    <textarea style={{ width: '1000px', height: '500px' }} name='cb_text' value={post.cb_text} onChange={handleChange}></textarea> <br /><br />
                    <button type='submit'>수정</button> &nbsp;
                    <button onClick={() => readPage('board_read', postNo)}>취소</button>
                </form>
            </div>
        </div>
    );
