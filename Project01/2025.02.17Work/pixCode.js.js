...
function BoardManagement({ readPage }) {
    ...
    function axiosGetAllPostList() {
        axios.get(`http://43.200.21.249:8080/nsd/companyBoard/getAllPostList?searchTag=${searchTag}&searchWord=${searchWord}&del=${del}&searchCategory=${searchCategory}`)
            .then((response) => {
    ...
    function recoverPost() {
        ...
        axios.patch('http://43.200.21.249:8080/nsd/companyBoard/recoverPost', { postNos: selectPosts }) //  키(key)는 'postNOs'이고 값(value)은 selectPosts배열을 담은 객체형태로 보냄
            .then(() => {
    ...
    function dataBaseDelPost() {    // 글 삭제시 해당 글에 달린 댓글 까지 전부 DB에서 삭제처리
        ...
        axios.delete('http://43.200.21.249:8080/nsd/companyBoard/dataBaseDelPost', { data: { postNos: selectPosts } })
            .then(() => {
                setSelectPosts([]);
                axiosGetAllPostList();
            })
    ...
    function addBoardCategory(e) {
        ...
        if (isDuplicateCategory) {
            ...
            axios.post(`http://43.200.21.249:8080/nsd/companyBoard/addCategory?name=${addCategory}`)
                .then(() => {
                    setAddCategory('');
                    axiosGetCategoryList();
                })
    ...
    function axiosGetCategoryList() {
        axios.get('http://43.200.21.249:8080/nsd/companyBoard/categoryList')
            .then((response) => {
    ...
    function deleteCategory(name) {
        ...
        axios.delete(`http://43.200.21.249:8080/nsd/companyBoard/deleteCategory?name=${name}`)
            .then(() => {
                axiosGetAllPostList();
                axiosGetCategoryList();
            })
    ...
    function onTtrendingPeriod(tp) {
        axios.patch(`http://43.200.21.249:8080/nsd/companyBoard/onTtrendingPeriod?tp=${tp}`)
            .then(() => {
                currentTrendingPeriod();
            })
            .catch(error => {
    ...
    function currentTrendingPeriod() {
        axios.get('http://43.200.21.249:8080/nsd/companyBoard/currentTrendingPeriod')
            .then((response) => {
            ...
            <div id='management_box'>
                        ...
                        {currentPosts.map((p, i) =>
                            <tr key={i}>
                                ...
                                {p.formattedDate === p.formattedModifyDate ?
                                    ...
                                    <td className='td2' style={{ width: '14%' }}>{formatDate(p.formattedModifyDate)}</td>
                                }
                ...
                <div id='button_box'>
                    ...
                    <div id='util_button_box'>
                        ...
                        <button className={!del ? 'select_button' : 'unSelect_button'} onClick={() => viewSelect()}>모든글 보기</button>
                        <button className={!del ? 'unSelect_button' : 'select_button'} onClick={() => viewSelect()}>삭제글 보기</button>
