...
function ProjectManagement({ loginRank }) {
    ...
    function pmExclude(id) {
        if(loginRank>=5){
            axios.post(`http://localhost:8080/spring/company/pmExclude?id=${id}`)
                .then(() => {
                    axiosGetProjectList();
                })
                .catch(error => {
    ...
    return (
        <div id='project_management_page'>
            <p>프로젝트 관리 페이지임(협업 프로젝트랑 같은페이지임)</p>
            <p>{loginRank}랭크 계정으로 접속중임(로그인 랭크에 따라 추가&종료 접근 가능함)</p>
