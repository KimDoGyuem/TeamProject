            ...
            <div id='management_box'>
                ...
                <div id='button_box'>
                    <div id='category_button_box'>
                        <button className={searchCategory === '' ? 'select_button' : ''} onClick={allList}>모든글 리스트</button>
                        {categoryList.map((c, i) =>
                            // <button key={i} onClick={() => setSearchCategory(c.cb_category_name)}>{c.cb_category_name}글 리스트</button>
                            <button className={searchCategory === c.cb_category_name ? 'select_button' : ''} key={i} onClick={() => setSearchCategory(c.cb_category_name)}>{c.cb_category_name}글 리스트</button>
                        )}
                    </div>
                ...
                <hr />
                <h3>인기글 관리</h3>
                <button className={period === 'day' ? 'select_button' : ''} value='day' onClick={(e) => onTtrendingPeriod(e.target.value)}>일간</button>
                <button className={period === 'week' ? 'select_button' : ''} value='week' onClick={(e) => onTtrendingPeriod(e.target.value)}>주간</button>
                <button className={period === 'month' ? 'select_button' : ''} value='month' onClick={(e) => onTtrendingPeriod(e.target.value)}>월간</button>
            </div>
        </div>
    );
}
