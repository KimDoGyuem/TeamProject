...
function BoardRead({ loginName, category, postNo, page, readPage }) {
    ...
    function axiosCommentsWrite(e) {
        ...
        if(comment === ''){
            ...
            let body = { cb_comment_no: postNo, company_id: sessionStorage.getItem('loginId'), company_name: loginName, cb_comment_text: comment }
            ...
            <div id='read_box'>
                ...
                {comments.map((c, i) =>
                    <div key={i}>
                        ...
                        {post.company_id === sessionStorage.getItem('loginId') && <button onClick={() => axiosDeleteComment(c.cb_comment_text)}>댓글삭제</button>} <br />
                    </div>
                )}
                <hr />
                ...
                {post.company_id === sessionStorage.getItem('loginId') && <button onClick={() => readPage('board_modify', postNo)}>글수정</button>} &nbsp;
                {post.company_id === sessionStorage.getItem('loginId') && <button onClick={() => axiosDeletePost()}>글삭제</button>}
            </div>
        </div>
    );
}
