...
function BoardManagement({ readPage }) {
    ...
    function recoverPost() {
        ...
        axios.patch('http://localhost:8080/spring/companyBoard/recoverPost', { postNos: selectPosts }) //  키(key)는 'postNOs'이고 값(value)은 selectPosts배열을 담은 객체형태로 보냄
            .then(() => {
    ...
    function dataBaseDelPost() {    // 글 삭제시 해당 글에 달린 댓글 까지 전부 DB에서 삭제처리
        ...
        axios.delete('http://localhost:8080/spring/companyBoard/dataBaseDelPost', { data: { postNos: selectPosts } })
            .then(() => {
                setSelectPosts([]);
                axiosGetAllPostList();
            })
    ...
    function deleteCategory(name) {
        ...
        axios.delete(`http://localhost:8080/spring/companyBoard/deleteCategory?name=${name}`)
            .then(() => {
                axiosGetAllPostList();
                axiosGetCategoryList();
            })
    ...
    function onTtrendingPeriod(tp) {
        axios.patch(`http://localhost:8080/spring/companyBoard/onTtrendingPeriod?tp=${tp}`)
            .then(() => {
                currentTrendingPeriod();
            })
            .catch(error => {
    ...
    function currentTrendingPeriod() {
        axios.get('http://localhost:8080/spring/companyBoard/currentTrendingPeriod')
            .then((response) => {
                console.log('api확인=', JSON.stringify(response.data, null, 2));
