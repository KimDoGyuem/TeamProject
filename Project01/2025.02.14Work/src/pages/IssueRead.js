...
function IssueRead() {
    ...
    function axiosIssueClose(e) {
        ...
        if(window.confirm(`"${issue.issue_no}"번 이슈를 닫으시겠습니까?`)){
            axios.patch(`http://localhost:8080/spring/company/issueClose?`, body)
                .then(()=>{
                    alert('!!issue Closed!!')
                    axiosGetIssueByIssueNo();
                })
    ...
    function axiosIssueReOpen(e) {
        ...
        if(window.confirm(`"${issue.issue_no}"번 이슈를 재오픈하시겠습니까?`)){
            axios.patch(`http://localhost:8080/spring/company/issueReOpen?issueNo=${issue_no}`)
                .then(()=>{
    ...
    function handleSubmitEdit(e) {
        ...
        axios.patch('http://localhost:8080/spring/company/issueCommentModify', editComment)
            .then(() => {
                setEditComment({ comment_no: 0, comment_text: '' });
                axiosGetIssueByIssueNo();
            })
